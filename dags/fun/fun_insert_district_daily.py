...
def fun_insert_district_daily():
    ...
    # get the value (total)
    df_melt = pd.melt(df, id_vars=['kode_kab', 'tanggal'],
    ...
    df_district_daily = pd.merge(df_melt, df_dim_case, on='status')
    df_district_daily.rename(columns={'kode_kab': 'district_id', 'tanggal': 'date'}, inplace=True) # modify column names
    df_district_daily['id'] = range(1, len(df_district_daily) + 1) #tambah kolom id
    df_district_daily = df_district_daily[['district_id', 'date', 'total', 'case_id', 'id']]

    # load district to postgresql
    table_name = "district_daily"
    df_district_daily.to_sql(table_name, pg_engine, if_exists='replace', index=False)
